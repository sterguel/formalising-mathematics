...
-- This is just Coursework 2 but cleaned up substantially!
...
-/
class integral_domain (R: Type) extends comm_ring R :=
(hzd: ∀ (x y : R), x * y = 0 → x = 0 ∨ y = 0)
...
-/
class pid (R: Type) extends integral_domain R :=
(hpid : ∀(I : myideal R), ∃ (x : R), I = principal_ideal x)
...
lemma symm_divisible_associates_int_domain (a b : S) : a \ b → b \ a → a ~ b:=
begin
  intros h1 h2,
  cases h1 with x hx,
  cases h2 with y hy,
  by_cases a ≠ 0,
  {
    ...
    rw mul_assoc at hy,
    rw ←mul_add at hy,
    have hint := _inst_2.hzd,
    specialize hint a (x * y + (-1)),
    have h2 : a = 0 ∨ x * y + -1 = 0,
    exact hint (eq.symm hy),
    cases h2,
...
lemma generators_associate_if_ideals_eq (a b : S) :
...
-/
class ufd (R : Type) extends integral_domain R :=
(hseq : ∀(f : ℕ → R), dividing_sequence f → ∃ (m : ℕ), ∀(q : ℕ), m ≤ q → f q ~ f (q + 1))
(hpi : ∀ (p: R), irreducible p →∀ (a b: R), p \ (a*b) → p \ a ∨ p \ b )
...
theorem pid_is_noetherian (P : Type) [pid P] 
(i :ℕ →  myideal P) (hinc : asc_ideal_chain i)  : 
...
theorem pid_irreducible_is_prime (P : Type) [pid P] (p : P) (hirr : irreducible p) :
  ∀ (a b : P), p \ (a * b) → p \ a ∨  p \ b :=
begin
  
  intros a b h,
  let I := sum_ideal (principal_ideal a) (principal_ideal p),
  have hpid := pid.hpid I,
...
{ 
  hseq := begin
    ...
    have hinc : asc_ideal_chain i,
      ...
      rw ← mul_assoc,
      exact hz,
    have hnoet := pid_is_noetherian P,
    specialize hnoet i,
